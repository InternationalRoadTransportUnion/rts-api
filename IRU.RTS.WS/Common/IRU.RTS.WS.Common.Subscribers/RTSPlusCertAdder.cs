        ...
        public void AddCertificate(X509Certificate2 certificate)
        {
            ...
            using (DbQueries dq = new DbQueries())
            {
                try
                {
                    ...
                    dq.InsertRtsplusSignatureKeyForSubscriber(_subscriberId, certificate.NotBefore, certificate.NotAfter, certificate.Thumbprint, certificate.Export(X509ContentType.Cert), sPrivKeyXml);
                }
                catch (Exception ex)
                {
