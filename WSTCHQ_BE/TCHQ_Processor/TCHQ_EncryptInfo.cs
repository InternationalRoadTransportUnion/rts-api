...
{	
	...
	public class TCHQ_EncryptInfo
	{
		...
		//	byte[] unicodeBytes = unicode.GetBytes(UnicodeEncoding.Default.GetString(cipher));// unicodeString);
			...
			success = TCHQ_InvokeCryptDll.Encrypt(asciiString, cipher, ref length);
		...
		public int EncryptInteger(int n_DecryptedInt)
		{
			...
			unsafe
			{
				uint len = sizeof(int);
				success  = TCHQ_InvokeCryptDll.InitializeCrypto();
				success  = TCHQ_InvokeCryptDll.EncryptNumFixedLength(ref n_DecryptedInt, ref n_CipherInt, ref len);
				success  = TCHQ_InvokeCryptDll.UninitializeCrypto();
			}
