	...
	BEGIN  
		...
		BEGIN 
			...
			BEGIN 
				...
				IF(( SELECT C_P_FACT_BULLETIN_DATE FROM CAR_CUR_PACKET_TABLE with (nolock) WHERE C_P_FACT_PKT_CARNET_NUMBER=(((CONVERT(INT,@TIRNumber/50))*50)+1) ) is not null) 
				BEGIN  
					SELECT CAR_CUR_CARNET_TABLE.C_C_CARNET_NUMBER as TIRNumber, 2 AS RESULT,  
					ISNULL(CAR_CUR_SYS_ASSOC_DETAILS.C_XST_ASSOCIATION_TXT,'IRU') as AssociationShortName,  
					(SELECT COUNT(*) FROM Dispatch.dbo.CAR_DISP_CARNETS with (nolock)  WHERE (Dispatch.dbo.CAR_DISP_CARNETS.D_C_CARNET_NUMBER = @TIRNumber)) AS NoOfDischarges 
