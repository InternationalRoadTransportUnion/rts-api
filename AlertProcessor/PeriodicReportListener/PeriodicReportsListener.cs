	...
	public class PeriodicReportsListener:IPlugIn, IRunnable 
	{
		...
		private bool PollReport()
		{
			...
			try
			{
                ...
                string sCommand = "SELECT     PERIODIC_REPORT.REPORT_ID, PERIODIC_REPORT.SERVICE_ID, PERIODIC_REPORT_SCHEDULE.NEXT_SCHEDULE_ID,  PERIODIC_REPORT_SQL.QUERY_SQL,PERIODIC_REPORT_SQL.QUERY_DESCRIPTION, PERIODIC_REPORT_SCHEDULE.SCHEDULE_DAY, PERIODIC_REPORT_SCHEDULE.SCHEDULE_TIME FROM         dbo.PERIODIC_REPORT with (nolock) INNER JOIN  dbo.PERIODIC_REPORT_SCHEDULE with (nolock)  ON PERIODIC_REPORT.REPORT_ID = PERIODIC_REPORT_SCHEDULE.REPORT_ID AND  PERIODIC_REPORT.NEXT_SCHEDULE_ID = PERIODIC_REPORT_SCHEDULE.SCHEDULE_ID INNER JOIN                       dbo.PERIODIC_REPORT_SQL with (nolock) ON PERIODIC_REPORT.REPORT_ID = PERIODIC_REPORT_SQL.REPORT_ID WHERE     (PERIODIC_REPORT.NEXT_EXECUTION_TIME <= @NOW)  OR                       (PERIODIC_REPORT.NEXT_EXECUTION_TIME IS NULL) ORDER BY PERIODIC_REPORT.REPORT_ID";
		...
		private void UpdateReportStatus(int ReportID, DateTime LastRun,DateTime NextTimeToRun, int NextScheduleID  )
		{
            ...
            string sUpdate = "UPDATE dbo.[PERIODIC_REPORT]   SET [NEXT_EXECUTION_TIME] = @NEXT_EXECUTION_TIME,  [LAST_EXECUTION_TIME] = @LAST_EXECUTION_TIME, [NEXT_SCHEDULE_ID] = @NEXT_SCHEDULE_ID       WHERE REPORT_ID=@REPORT_ID";
