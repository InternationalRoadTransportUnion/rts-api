            ...
            using (NetTcpClient<ICryptoOperations> client = new NetTcpClient<ICryptoOperations>(EGIS_RemotingHelper.m_CryptoProviderEndpoint))
            {
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        carnetingHashTable = carnetingSvcHelper.GetCarnetDetailsAndSafeTIRData(egisRequestLogData.tirCarnetNumber, (egisRequestLogData.queryType & 0x2) == 0x2);
                        iCarnetingTCHQResultCode = int.Parse(carnetingHashTable["Query_Result_Code"].ToString());
                    }
                    catch (Exception e)
                    {
                    ...
                    #region Query to Transit Messaging Service (BizTalk eTIR)

                    try
                    {
                        EGIS_TransitMessagingServiceHelper transitMessagingSvcHelper = new EGIS_TransitMessagingServiceHelper(EGIS_RemotingHelper.m_TransitMessagingServiceEndPoint);
                        epdMessagingHashTable = transitMessagingSvcHelper.GetTransitMessagesOfGuarantee(egisRequestLogData.tirCarnetNumber);
                    }
                    catch (Exception e)
                    {
                        Statics.IRUTrace(this, Statics.IRUTraceSwitch.TraceWarning,
                            "Invalid value whilst retrieving the EPD Data from the Transit Messaging Service in EGIS_Processor.EGIS_TransitMessagingServiceHelper.cs"
                            + e.Message);
                        throw e;
                    }
                    ...
                    nodeResponseBody.SelectSingleNode("./tchq:Result", xnsResponse).InnerText = carnetingHashTable["Query_Result_Code"].ToString();
                    nodeResponseBody.SelectSingleNode("./tchq:Carnet_Number", xnsResponse).InnerText = egisRequestLogData.tirCarnetNumber;

                    nodeResponseBody.SelectSingleNode("./tchq:HolderID", xnsResponse).InnerText = GetBlankIfNull(carnetingHashTable["Holder_ID"]);
                    ...
                    nodeResponseBody.SelectSingleNode("./tchq:ValidityDate", xnsResponse).InnerText = GetBlankDateIfNull(carnetingHashTable["Validity_Date"]);
                    ...
                    nodeResponseBody.SelectSingleNode("./tchq:Association", xnsResponse).InnerText = GetBlankIfNull(carnetingHashTable["Assoc_Short_Name"]);
                    ...
                    nodeResponseBody.SelectSingleNode("./tchq:NumTerminations", xnsResponse).InnerText = GetBlankIfNull(carnetingHashTable["No_Of_Terminations"]);
                    ...
                    nodeResponseBody.SelectSingleNode("./tchq:Voucher_Number", xnsResponse).InnerText = GetBlankIfNull(carnetingHashTable["Voucher_Number"]);
                    ...
                    egisRequestLogData.queryResultCode = Int32.Parse(carnetingHashTable["Query_Result_Code"].ToString());

                    egisRequestLogData.holderID = GetBlankIfNull(carnetingHashTable["Holder_ID"]);
                    egisRequestLogData.validityDate = carnetingHashTable["Validity_Date"];
                    egisRequestLogData.assocShortName = GetBlankIfNull(carnetingHashTable["Assoc_Short_Name"]);
