...
{	
	...
	public class EGIS_EncryptInfo
	{
		...
		//	byte[] unicodeBytes = unicode.GetBytes(UnicodeEncoding.Default.GetString(cipher));// unicodeString);
			...
			success = EGIS_InvokeCryptDll.InitializeCrypto();
			success = EGIS_InvokeCryptDll.Encrypt(asciiString, cipher, ref length);
			success = EGIS_InvokeCryptDll.UninitializeCrypto();
		...
		public int EncryptInteger(int n_DecryptedInt)
		{
			...
			unsafe
			{
				uint len = sizeof(int);
				success  = EGIS_InvokeCryptDll.InitializeCrypto();
				success  = EGIS_InvokeCryptDll.EncryptNumFixedLength(ref n_DecryptedInt, ref n_CipherInt, ref len);
				success  = EGIS_InvokeCryptDll.UninitializeCrypto();
			}
