...
namespace EGIS_Processor
{
	...
	public class EGIS_DecryptInfo
	{
		...
		public string DecryptString(string s_CipherText, uint length)
		{
			...
			success = EGIS_InvokeCryptDll.Decrypt(decryptData, cipher, ref length);
		...
		public string DecryptInteger(int n_CipherInt)
		{
			...
			unsafe
			{
				uint len = sizeof(int);
				success  = EGIS_InvokeCryptDll.InitializeCrypto();
				success  = EGIS_InvokeCryptDll.DecryptNumFixedLength(ref n_DecryptedInt, ref n_CipherInt, ref len);
				success  = EGIS_InvokeCryptDll.UninitializeCrypto();
			}
