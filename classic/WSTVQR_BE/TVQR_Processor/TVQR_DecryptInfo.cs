...
namespace TVQR_Processor
{
	...
	public class TVQR_DecryptInfo
	{
		...
		public string DecryptString(string s_CipherText, uint length)
		{
			...
			success = TVQR_InvokeCryptDll.Decrypt(decryptData, cipher, ref length);
		...
		public string DecryptInteger(int n_CipherInt)
		{
			...
			unsafe
			{
				uint len = sizeof(int);
				success  = TVQR_InvokeCryptDll.InitializeCrypto();
				success  = TVQR_InvokeCryptDll.DecryptNumFixedLength(ref n_DecryptedInt, ref n_CipherInt, ref len);
				success  = TVQR_InvokeCryptDll.UninitializeCrypto();
			}
