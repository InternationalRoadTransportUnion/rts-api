...
{
	/// <summary>
	/// Summary description for WSRQStressTester.
	/// </summary>
	public class WSRQStressTester : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
            ...
            this.Name = "WSRQStressTester";
            this.Text = "WSRQStressTester";
		...
		private void linkLabel1_LinkClicked(object sender, System.Windows.Forms.LinkLabelLinkClickedEventArgs e)
		{
			...
			try
			{
				...
				WSRQStressTest tcs = new WSRQStressTest();
		...
		private void generateEncFile(string sFileName, RSAParameters rParams)
		{
			...
			/*
				...
				*/
				...
				bodyContents = RegExHelper.ExtractBODYContents(sQueryXML.Trim());
                bodyContents = r.Replace(bodyContents.Trim(), " ");
				byte[] aBodyContents = System.Text.Encoding.Unicode.GetBytes(bodyContents.Trim());
		...
		private void linkLabel3_LinkClicked(object sender, System.Windows.Forms.LinkLabelLinkClickedEventArgs e)
		{
			...
			try
			{
				...
				foreach (string[] aFileQID in m_aStressData)
				{
					...
					string sFileName =  aFileQID[0] + ".xml";
					string sFilePath = m_messageGenPath + "\\" + sFileName;
                    CallWS(sFilePath, m_thumbPrint, aFileQID[0], aFileQID[1], aFileQID[2], m_rsaParams, chkResponseVerfication.Checked);			 
		...
		private void CallWS(string sFileName,string Thumbprint, string sQueryID, string SubscriberID,string Information_Exchange_Version ,RSAParameters rParams, bool bRespVerification)
		{
			WSRQ.ReconciliationQuery tq = new WSRQ.ReconciliationQuery();
           
			//read encsession key file

			string fileName =sFileName+".3DesEncKey.bin";

			tq.ESessionKey = WSRQTest.GetFileContents(fileName);

			tq.MessageTag=Thumbprint;

			fileName = sFileName+".encrypted.bin";
			tq.ReconciliationQueryData = WSRQTest.GetFileContents(fileName);
			tq.SubscriberID = SubscriberID;
            tq.Sender_MessageID = sQueryID;
            tq.Information_Exchange_Version = Information_Exchange_Version;
		
			Application.DoEvents();

			WSRQ.ReconciliationQueryServiceClass  queryClass= new WSRQ.ReconciliationQueryServiceClass();

			WSRQ.ReconciliationResponse tr ;
			
			DateTime dtStart, dtEnd;

			dtStart = DateTime.Now;

			tr= queryClass.WSRQ(tq);
			dtEnd = DateTime.Now;
			TimeSpan ts = dtEnd- dtStart;
			
			AppendStressMessage( " \r\n" + Thread.CurrentThread.Name + "|" +sFileName +"|"+ sQueryID + " |" +   "The Query ID is |" + tr.Sender_MessageID.ToString() + " |The return code is |" + tr.ReturnCode.ToString() + " |The Message Tag is |" +  tr.MessageTag + " |Response time is (ms)|" + ts.TotalMilliseconds.ToString());
			...
			try
			{
				decoutputMsg = ic.Decrypt(tr.ReconciliationRequestData,"3DES",ht3);
			}
			catch  (Exception crypEx)
			{
				//MessageBox.Show(" \r\n"  + sQueryID + " -- " + "Crypt Exception: Check log file for details: " + crypEx.Message);
