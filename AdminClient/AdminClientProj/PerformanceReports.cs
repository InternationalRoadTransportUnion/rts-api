...
namespace IRU.RTS.AdminClient
{
	...
	public class PerformanceReports : System.Windows.Forms.Form
	{
        ...
        SqlCommand PrepareWSRQQuery()
        {
            ...
            string sReportSQL =
                "SELECT  WIL.ROW_CREATED_TIME, WIL.RESPONSE_TIME,  WSRQ_STEP, WSEQ.LAST_UPDATE_TIME FROM     " +
                "dbo.WSRQ_LOG WIL WITH (NOLOCK) INNER JOIN dbo.WSRQ_SEQUENCE WSEQ  WITH (NOLOCK) ON  WIL.EXCHANGEID = WSEQ.EXCHANGEID  " +
                "WHERE ";

            sWhereSQL += "AND SUBSCRIBER_ID = @SENDER_ID " +
                "AND ROW_CREATED_TIME<=@ROW_CREATED_TIME_END ";

            sWhereSQL += "AND WSRQ_STEP>=@WSRQ_STEP_START AND WSRQ_STEP<=@WSRQ_STEP_END ";
            ...
            sWhereSQL += " AND WIL.RESPONSE_TIME IS NOT NULL  ORDER BY WIL.ROW_CREATED_TIME, WIL.EXCHANGEID, WSRQ_STEP ";

            #region mandatory params

            sParam = new SqlParameter("@WSRQ_STEP_START", SqlDbType.Int);
            sParam.Value = ((StepCodeDescriptionMap)cmbStep.SelectedItem).Step;
            sqlReportCmd.Parameters.Add(sParam);

            sParam = new SqlParameter("@WSRQ_STEP_END", SqlDbType.Int);
            sParam.Value = ((StepCodeDescriptionMap)cmbEndStep.SelectedItem).Step;
            sqlReportCmd.Parameters.Add(sParam);
			...
			string sReportSQL=
                ...
                "dbo.WSST_INTERNAL_LOG WIL WITH (NOLOCK) INNER JOIN dbo.WSST_SEQUENCE WSEQ  WITH (NOLOCK) ON  WIL.SAFETIR_MESSAGE_IN_ID = WSEQ.SAFETIR_MESSAGE_IN_ID  " + 
"WHERE " ;
...
sWhereSQL +=" AND WIL.COMPLETION_TIME IS NOT NULL  ORDER BY WIL.ROW_CREATED_TIME, WIL.SAFETIR_MESSAGE_IN_ID, WSST_STEP "; 
        ...
        SqlCommand PrepareWSREInternalQuery()
        {
            ...
            string sReportSQL =
                "SELECT      WIL.ROW_CREATED_TIME, WIL.COMPLETION_TIME,  WSRE_STEP, WSEQ.LAST_UPDATE_TIME FROM     " +
                "dbo.WSRE_INTERNAL_LOG WIL WITH (NOLOCK) INNER JOIN dbo.WSRE_SEQUENCE WSEQ  WITH (NOLOCK) ON  WIL.SAFETIR_MESSAGE_IN_ID = WSEQ.SAFETIR_MESSAGE_IN_ID  " +
"WHERE ";
            ...
            sWhereSQL += "AND WSRE_STEP>=@WSRE_STEP_START AND WSRE_STEP<=@WSRE_STEP_END ";
            ...
            sWhereSQL += " AND WIL.COMPLETION_TIME IS NOT NULL  ORDER BY WIL.ROW_CREATED_TIME, WIL.SAFETIR_MESSAGE_IN_ID, WSRE_STEP ";

            #region mandatory params

            sParam = new SqlParameter("@WSRE_STEP_START", SqlDbType.Int);
            sParam.Value = ((StepCodeDescriptionMap)cmbStep.SelectedItem).Step;
            sqlReportCmd.Parameters.Add(sParam);

            sParam = new SqlParameter("@WSRE_STEP_END", SqlDbType.Int);
            sParam.Value = ((StepCodeDescriptionMap)cmbEndStep.SelectedItem).Step;
            sqlReportCmd.Parameters.Add(sParam);
