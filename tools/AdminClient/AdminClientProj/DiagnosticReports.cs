...
namespace IRU.RTS.AdminClient
{
	...
	public class DiagnosticReports : System.Windows.Forms.Form
	{
		...
		private void btnSvcSelect_Click(object sender, System.EventArgs e)
		{
            ...
            if ((ConfigurationSettings.AppSettings["Location"].ToString() == "BlueLan") && (cmbServices.Text == "WSST" || cmbServices.Text == "WSRE") )
			{
            ...
            string sStepSelect = "SELECT STEP, STEP_DESCRIPTION FROM dbo.STEP_DEFINITIONS ORDER BY STEP";
		...
		private void FillStepResultCombo()
		{
            ...
            string sStepREsultSelect = "SELECT  STEP_RESULT , STEP_RESULT_DESCRIPTION FROM dbo.STEP_RESULT_DEFINITIONS WHERE STEP=@STEP";
		...
		private void ExecuteReport()
		{
			...
			switch (cmbServices.Text)
			{
                ...
                case "WSRE":
                    ...
                    {
                        //there is no sequence table
                        sRepCommand = PrepareWSREExternalQuery();
                    }
                    else
                    {

                        sRepCommand = PrepareWSREInternalQuery();
                    }
                    break;
		...
		SqlCommand PrepareTCHQQuery()
		{
			...
			string sReportSQL=
                ...
                "dbo.TCHQ_REQUEST_LOG WIL WITH (NOLOCK) INNER JOIN dbo.TCHQ_SEQUENCE WSEQ  WITH (NOLOCK) ON  WIL.TCHQ_QUERY_ID = WSEQ.TCHQ_QUERY_ID  " + 
				"WHERE " ;
			...
			if (cmbStepResults.Text!="ALL") //if specific step result is not expected then no need to fileter the sequence table
			{
				if (cmbStep.Text !="ALL")
				{
                    sWhereSQL += "AND WIL.TCHQ_QUERY_ID IN (SELECT TCHQ_QUERY_ID FROM dbo.TCHQ_SEQUENCE  WITH (NOLOCK) WHERE  " + " TCHQ_STEP=@O_TCHQ_STEP ";
        ...
        SqlCommand PrepareWSRQQuery()
        {
            ...
            string sReportSQL =
                "SELECT WIL.EXCHANGEID,  WIL.SENDER_MESSAGEID, WSRQ_STEP, WSEQ.WSRQ_STEP_RESULT, " +
                "REPLACE(CONVERT(NVARCHAR(4000),WSEQ.WSRQ_STEP_ERROR_DESC),',','_') as WSRQ_STEP_ERROR_DESC, CONVERT(nvarchar(19),WSEQ.LAST_UPDATE_TIME ,120) AS LAST_UPDATE_TIME FROM     " +
                "dbo.WSRQ_LOG WIL WITH (NOLOCK) INNER JOIN dbo.WSRQ_SEQUENCE WSEQ  WITH (NOLOCK) ON  WIL.EXCHANGEID = WSEQ.EXCHANGEID  " +
                "WHERE ";
            ...
            if (cmbStepResults.Text != "ALL") //if specific step result is not expected then no need to fileter the sequence table
            {
                if (cmbStep.Text != "ALL")
                {
                    sWhereSQL += "AND WIL.EXCHANGEID IN (SELECT EXCHANGEID FROM dbo.WSRQ_SEQUENCE  WITH (NOLOCK) WHERE  " + " WSRQ_STEP=@O_WSRQ_STEP ";


                    sParam = new SqlParameter("@O_WSRQ_STEP", SqlDbType.Int);
                    sParam.Value = ((StepCodeDescriptionMap)(cmbStep.SelectedItem)).Step;
                    sqlReportCmd.Parameters.Add(sParam);

                    if (cmbStepResults.Text != "ALL")
                    {
                        sWhereSQL += "AND WSRQ_STEP_RESULT=@O_WSRQ_STEP_RESULT ";

                        sParam = new SqlParameter("@O_WSRQ_STEP_RESULT", SqlDbType.Int);
                        sParam.Value = ((StepCodeDescriptionMap)(cmbStepResults.SelectedItem)).Step;
                        sqlReportCmd.Parameters.Add(sParam);
                    ...
                    if (txtMessageID.Text != "ALL")
                    {
                        sWhereSQL += "AND EXCHANGEID = @O_WSRQ_QUERY_ID ";
                        sParam = new SqlParameter("@O_WSRQ_QUERY_ID", SqlDbType.Int);
                        sParam.Value = int.Parse(txtMessageID.Text);
                        sqlReportCmd.Parameters.Add(sParam);
                    }
            ...
            if (txtMessageID.Text != "ALL")
            {
                sWhereSQL += "AND WIL.EXCHANGEID = @O_WSRQ_QUERY_ID_2 ";
                sParam = new SqlParameter("@O_WSRQ_QUERY_ID_2", SqlDbType.Int);
                sParam.Value = int.Parse(txtMessageID.Text);
                sqlReportCmd.Parameters.Add(sParam);
            }


            sWhereSQL += "AND DATEDIFF(s,ROW_CREATED_TIME,RESPONSE_TIME)>=@THRESHOLD " +
                " AND SUBSCRIBER_ID = @SENDER_ID " +
            ...
            sWhereSQL += "ORDER BY WIL.EXCHANGEID, WSEQ.WSRQ_STEP ";
			...
			string sReportSQL=
                ...
                "dbo.WSST_INTERNAL_LOG WIL WITH (NOLOCK) INNER JOIN dbo.WSST_SEQUENCE WSEQ  WITH (NOLOCK) ON  " +
				...
				if (cmbStepResults.Text!="ALL") //if specific step result is not expected then no need to fileter the sequence table
				{
					if (cmbStep.Text !="ALL")
					{
                        sWhereSQL += "AND WIL.SAFETIR_MESSAGE_IN_ID IN (SELECT SAFETIR_MESSAGE_IN_ID FROM dbo.WSST_SEQUENCE  WITH (NOLOCK) WHERE  " + " WSST_STEP=@O_WSST_STEP ";
        ...
        SqlCommand PrepareWSREInternalQuery()
        {
            ...
            string sReportSQL =
                "SELECT WIL.SAFETIR_MESSAGE_IN_ID,  WIL.SENDER_MESSAGE_ID, WSEQ.WSRE_STEP, WSEQ.WSRE_STEP_RESULT, " +
                "REPLACE(CONVERT(NVARCHAR(4000),WSEQ.WSRE_STEP_ERROR_DESCRIPTION),',','_') as WSRE_STEP_ERROR_DESCRIPTION, " +
                "CONVERT(nvarchar(19),WSEQ.LAST_UPDATE_TIME ,120) AS LAST_UPDATE_TIME FROM " +
                "dbo.WSRE_INTERNAL_LOG WIL WITH (NOLOCK) INNER JOIN dbo.WSRE_SEQUENCE WSEQ  WITH (NOLOCK) ON  " +
                "WIL.SAFETIR_MESSAGE_IN_ID = WSEQ.SAFETIR_MESSAGE_IN_ID WHERE ";
            ...
            if (cmbStepResults.Text != "ALL") //if specific step result is not expected then no need to fileter the sequence table
            {
                if (cmbStep.Text != "ALL")
                {
                    sWhereSQL += "AND WIL.SAFETIR_MESSAGE_IN_ID IN (SELECT SAFETIR_MESSAGE_IN_ID FROM dbo.WSRE_SEQUENCE  WITH (NOLOCK) WHERE  " + " WSRE_STEP=@O_WSRE_STEP ";


                    sParam = new SqlParameter("@O_WSRE_STEP", SqlDbType.Int);
                    sParam.Value = ((StepCodeDescriptionMap)(cmbStep.SelectedItem)).Step;
                    sqlReportCmd.Parameters.Add(sParam);

                    if (cmbStepResults.Text != "ALL")
                    {
                        sWhereSQL += "AND WSRE_STEP_RESULT=@O_WSRE_STEP_RESULT ";

                        sParam = new SqlParameter("@O_WSRE_STEP_RESULT", SqlDbType.Int);
                        sParam.Value = ((StepCodeDescriptionMap)(cmbStepResults.SelectedItem)).Step;
                        sqlReportCmd.Parameters.Add(sParam);
            ...
            sWhereSQL += "ORDER BY WIL.SAFETIR_MESSAGE_IN_ID, WSEQ.WSRE_STEP ";
		...
		SqlCommand PrepareWSSTExternalQuery()
		{
			...
			string sReportSQL=
                ...
                "dbo.WSST_EXTERNAL_LOG WIL WITH (NOLOCK) " + 
				"WHERE " ;
        ...
        SqlCommand PrepareWSREExternalQuery()
        {
            ...
            string sReportSQL =
                "SELECT      WIL.SAFETIR_MESSAGE_IN_ID,  WIL.SENDER_MESSAGEID, RETURN_CODE, REPLACE(CONVERT(NVARCHAR(4000),RETURN_CODE_DESCRIPTION),',','_') as RETURN_CODE_DESCRIPTION, CONVERT(nvarchar(19),ROW_CREATED_TIME ,120) AS ROW_CREATED_TIME FROM     " +
                "dbo.WSRE_EXTERNAL_LOG WIL WITH (NOLOCK) " +
                "WHERE ";
            ...
            if (cmbStepResults.Text != "ALL") //if specific step result is not expected then no need to fileter the sequence table
            {
                ...
                sWhereSQL += "AND RETURN_CODE =@O_WSRE_STEP_RESULT ";
		...
		SqlCommand PrepareWSCsccQuery()
		{
			...
			string sReportSQL=
                ...
                "dbo.WSST_COPY_TO_LOG WIL WITH (NOLOCK) INNER JOIN dbo.WSST_COPY_TO_SEQUENCE WSEQ  WITH (NOLOCK) ON  WIL.SAFETIR_MESSAGE_IN_ID = WSEQ.SAFETIR_MESSAGE_IN_ID  " + 
				"WHERE " ;
			...
			if (cmbStepResults.Text!="ALL") //if specific step result is not expected then no need to fileter the sequence table
			{
				if (cmbStep.Text !="ALL")
				{
                    sWhereSQL += "AND WIL.SAFETIR_MESSAGE_IN_ID IN (SELECT SAFETIR_MESSAGE_IN_ID FROM dbo.WSST_COPY_TO_SEQUENCE  WITH (NOLOCK) WHERE  " + " COPY_STEP=@O_COPY_STEP ";
